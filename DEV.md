...
`interview.py` — спецификации 18 вопросов и совместимость прежнего представления.
`editorial_update.py` — редактура и единый формат. `main.py` — Telegram/AI вызовы.
...
Ментор использует `OPENAI_MODEL`, timeout 35 s и no retry. Раунды различаются:
эпизод/чувства → личная значимость → ещё неясный запрос или противоречие.
`compose_edited_update` вызывает редактор с timeout 120 s, 11000 output tokens,
medium reasoning и JSON output. Проверяются completed status, структура, дословное
наличие ссылочных цитат и явно названные оценки /10. Цитаты обеспечивают
проверяемое происхождение; смысловую точность перефразирования не гарантирует
одна лишь проверка substring. Поэтому второй вызов независимо сверяет смысл
с оригиналами: отрицания, сравнения, числа, авторство чувств, причинные связи.
Он возвращает только точечные замены по строгой JSON Schema (timeout 75 s,
6000 tokens, medium reasoning); исправления снова проходят проверку цитат.
Для ремонтируемых файлов дополнительно нужна вычитка.
Сбой оставляет активный черновик и кнопку повторного сохранения, без экспорта.

Новый Markdown объединяет обе методики внутри трёх сфер и одного запроса.
Версионированный HTML-comment `forum-update-source-v1` содержит raw answers,
mentor и clean_answers; он исключён из HTML и контекста анализа динамики.
Подсказки/редактирование используют clean_answers; старый Q&A формат остаётся
читаемым. HTML экранирует данные и не обрезает текст, не запускает второй AI-проход.

Ремонт конкретного готового апдейта на сервере:
`python repair_update.py USER_ID FILENAME` создаёт только кандидата в
`data/repairs/USER_ID/STEM/`, исходный Markdown и прежний HTML сохраняются рядом.
После вычитки candidate.md и candidate.html команда с `--apply` сверяет хэши,
заменяет выбранный файл под тем же ID, обновляет SQLite/HTML-кэш и экспорт профиля.
Другие апдейты не затрагиваются. Старые снимки экспорта переходят в `superseded/`
с явной пометкой; корневой README перечисляет только текущие версии.
